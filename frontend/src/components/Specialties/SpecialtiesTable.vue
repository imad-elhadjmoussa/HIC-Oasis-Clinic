...
// Filter specialties based on name search
const filteredSpecialties = computed(() => {
    if (!searchName.value) return props.specialties;
    return props.specialties.filter(specialty =>
        specialty.specialty_name.toLowerCase().includes(searchName.value.toLowerCase())
    );
});
...
<template>
    <div class="mb-4">
        <span class="p-input-icon-left">
            <InputText v-model="searchName" placeholder="Search by name..." class="w-full md:w-80" />
        </span>
    </div>

    <DataTable :value="filteredSpecialties" :paginator="true" class="p-datatable-sm" :rows="4" :loading="loading" stripedRows>
        <template #empty>

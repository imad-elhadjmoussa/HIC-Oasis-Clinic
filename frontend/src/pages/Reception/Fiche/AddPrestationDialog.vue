...
const addPrestation = async () => {
    ...
    try {
        ...
        props.prestations.push(
            response.data
        )

        // emit('add-prestation', {
        //     ...response.data,
        //     specialty_name: currentSpecialtyDetails.value.specialty_name,
        //     name: selectedPrestation.value.name,
        //     unit_price: selectedPrestation.value.price,
    ...
    <Dialog :visible="modelValue" @update:visible="$emit('update:modelValue', $event)" header="Add Medical Prestation"
        ...
        <div class="grid gap-4">
            ...
            <div class="field">
                ...
                <Dropdown id="doctor" v-model="newPrestation.doctor" :options="currentSpecialtyDetails.doctors"
                    optionLabel="doctor_first_name" optionValue="doctor_id" placeholder="Select Doctor" class="w-full"
                    :disabled="!newPrestation.specialty" />
            </div>

            <!-- Prestation summary -->
            <div v-if="selectedPrestation" class="bg-blue-50 p-4 rounded-lg">
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <p class="text-sm font-medium text-gray-500">Selected Prestation</p>
                        <p class="font-medium">{{ selectedPrestation.name }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">Specialty</p>
                        <p>{{ currentSpecialtyDetails.specialty_name }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-500">Price</p>
                        <p>{{ selectedPrestation.price }} DH</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <template #footer>
            ...
            <Button label="Add Prestation" icon="pi pi-plus" @click="addPrestation"
                :disabled="!newPrestation.specialty || !newPrestation.prestation" class="p-button-success" />
        </template>
    </Dialog>
</template>